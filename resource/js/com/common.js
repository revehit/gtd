...
$(function(){
	...
	$('#header .gnb_wrap > ul > li').hover(function(){
		$('#header').toggleClass('drop', 300);
		$(this).toggleClass('on', 300);
		$(this).children('.gnb_depth2').stop().slideToggle(300);
	});
	...
	//	$(this).closest('.tab_ui').find('.tab_cont > ul > li').eq(idx).addClass('active').siblings().removeClass('active');
	//});
	//파일업로드
...
$(function(){
	$(window).on("load",function(){
		...
		$("#content-d1").mCustomScrollbar({theme:"dark-2"});
	});
	//footer site btn
	$('.select_type02 > a').on('click',function(){
		var SelectType2 = $(this).parents('.select_type02');	
		SelectType2.find('.select_con').stop().slideToggle();
		SelectType2.toggleClass('active');
	});
});

$(document).mouseup(function (e){
	  //footer site btn
	  var SelectType02 = $(".select_type02.active");
	  if(SelectType02.has(e.target).length === 0){
		  $('.select_type02').removeClass('active');
		  $(".select_type02 .select_con").stop().slideUp();
	  }
	});
